...
class DynamicDistLLMMemory:
    def __init__(self, instructions, contents, node_id=None, on_send=None, on_receive=None):
        ...
        self.contentTrigger = TriggerManager() # manage LLM content callbacks, called when event updates content
    ...
    def update_content_json(self, anchor, json_str: str, replace=False) -> None:
        ...
        if isinstance(json_str, str):
            try:
                ...
                print(f"Parsed new_data from string: {new_data} (type: {type(new_data)})")
            except json.JSONDecodeError as e:
        ...
        if anchor in self.contents:
            ...
            print(f"Existing content before update: {existing_content} (type: {type(existing_content)})")

            # Parse existing content if it's a JSON string
            if isinstance(existing_content, str):
                try:
                    existing_content = json.loads(existing_content)
                    print(f"Parsed existing_content: {existing_content} (type: {type(existing_content)})")
                except json.JSONDecodeError as e:
        ...
        print(f"Updated content at {anchor}: {self.contents[anchor]} (type: {type(self.contents[anchor])})")
...
# MEMORY OPTIMIZATION
    ...
    def add_operation(self, operations, command, counter, read_object, write_object):
        ...
        try:
            ...
            operation = {'command': unique_command, 'original_command': command, 'read': reads, 'write': write_object}
    ...
    def identify_end_write_objects(self,graph):
        ...
        end_write_objects = {node for node in graph.nodes if graph.out_degree(node) == 0}
    ...
    def find_partitionable_subgraphs(self,graph, end_write_objects):
        ...
        for end_obj in end_write_objects:
            subgraph_nodes = [node for node in graph.nodes if node == end_obj or end_obj in nx.descendants(graph, node)]
            if subgraph_nodes:
...
#                counter+=1
                ...
                for item in scopestring.split():
                    reads.append(item)
                self.add_operation(operations,'LLM-CONTENT-TRIGGER', counter,reads, anchor)      
                counter+=1
 #           elif instruction['command'] == 'COMPRESS':
...
#                scopestring = scope
 ...
 #               counter+=1
        ...
        partitions = self.traverse_from_end_write_objects(graph_without_end_nodes, call_graph, end_write_objects)
    ...
    def get_clusters_of_single_write_endpoints(self, contents):
        ...
        for content_key in contents.keys():
            ...
            if len(content_reads) == 1 and len(content_writes) > 1:
                ...
                cluster = {"content": content_key,
                            "operations": [{"type": "READ", "target": content_key},
    ...
    def ask_litellm(self, prompt, llm_name=None):
        try:
            ...
            if 'reflection' in llm_name:
                ...
                final_response = self.extract_output_content(response.choices[0].message.content)
